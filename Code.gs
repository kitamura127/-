...
function removeUnnecessaryInfo(companyName, address) {
  ...
  const industryKeywords = [
    '建設', '工務', '土木', '解体', '鉄筋', 'とび', '型枠', '配管', '内装',
    '足場', '電気', '設備', '塗装', '防水', '左官', '大工', '板金', '屋根',
    '外構', '造園', '舗装', 'リフォーム', '建築', '住宅', '工事', '施工'
  ];
...
function showIntelligenceInsights() {
  ...
  if (insights.urgent_opportunities && insights.urgent_opportunities.length > 0) {
    insights.urgent_opportunities.forEach((opp, index) => {
      const priorityColor = opp.priority === '高' ? '#f44336' : opp.priority === '中' ? '#ff9800' : '#4caf50';
      const typeEmoji = opp.type.includes('繁忙') ? '🔥' : opp.type.includes('物件') ? '📅' : opp.type.includes('施工') ? '🏢' : '💰';
