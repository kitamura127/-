...
/***** 訴求内容生成機能 *****/
function generateSalesProposal(customerIssue) {
  ...
  // 会社の強みリストを整形
  let strengthsList = '';
  for (const key in COMPANY_STRENGTHS) {
    const strength = COMPANY_STRENGTHS[key];
    strengthsList += `■${strength.name}\n${strength.detail}\n\n`;
  }

  const prompt = `あなたは建設作業員派遣会社の営業アシスタントです。
お客様が抱えているお困りごとに対して、当社の強みを活かした効果的な訴求内容を提案してください。
...
以下の形式でJSON形式のみで回答してください。他の文章は一切含めないでください：

{
  "proposals": [
    {
      "strength_name": "活用する強みの名前（例：価格、動員力など）",
      "appeal_point": "80文字以内の訴求ポイント",
      "detail": "150文字以内の具体的な提案内容"
    }
  ],
  "summary": "100文字以内の総合的な提案サマリー"
}

※最も効果的な強みを3つ選んで提案してください。
※お客様の課題解決に直結する内容にしてください。
※具体的な数字やメリットを含めてください。`;
  ...
  try {
    ...
    Logger.log(`AI Response: ${aiResponseText.substring(0, 500)}`);
    ...
    try {
      ...
      if (!parsed.proposals || !Array.isArray(parsed.proposals)) {
        ...
        return {
          proposals: [],
          summary: '提案の生成に失敗しました。'
        };
      }

      Logger.log(`Generated ${parsed.proposals.length} proposals`);
      return parsed;
...
/***** メモ更新 *****/
function updateMemo(row, newMemo) {
  const sheet = getTargetSheet();
  updateCell(sheet, row, CONFIG.COLUMNS.MEMO, newMemo);
  return newMemo;
}
