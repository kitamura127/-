...
/***** Gemini API分析 - 修正版（gemini-2.0-flash-exp対応）*****/
...
function parseKentsuEmail(message) {
  ...
  lines.forEach(line => {
    ...
    if (line.startsWith('●')) {
      ...
      current = { client: line.substring(1).trim(), projectName: '', address: '', price: 0, emailDate: message.getDate() };
